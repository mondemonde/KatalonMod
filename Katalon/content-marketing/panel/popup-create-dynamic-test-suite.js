...
export async function checkLoginOrSignupUserForCreateDynamicTestSuite(){
    ...
    const onboardingChoice = (await browser.storage.local.get("onBoardingUserChoice")).onBoardingUserChoice;
    const leftSidePanelData = await getTrackingLeftSidePanelData();

    if (!loginData?.isActived && leftSidePanelData.addDynamicTestSuite >= createDynamicThreshold  && onboardingChoice.use_case === "kickstart-automation"){
