...
const addProfile = async (profile) => {
  if (profile === undefined || profile === null) {
    throw "Null or undefined profile";
  }
  if (profile.isDefault){
    let currentDefaultProfile;
    try{
      currentDefaultProfile = await getDefaultProfile();
    } catch (e){}
    if (currentDefaultProfile){
      throw "Cannot 2 default profiles";
    }
  }
...
const removeProfile = async (profileID) => {
  ...
  for (let i = 0; i < profileData.profiles.length; i++) {
    ...
    if (profile.id === profileID) {
      if (profile.isDefault){
        throw "Cannot delete a default profile";
      }
...
const getDefaultProfile = async () => {
  ...
  if (defaultProfile === undefined){
    throw "Undefined default profile";
  }
