...
function trackingExecuteTestSuites(type, isSelfHealing) {
    ...
    switch (type) {
        case 'suite':
            trackingExecuteTestSuite(title, numSuccessedTestCase, numFailedTestCase, isSelfHealing, undefined)
            break;
        case 'all':
            trackingExecuteAll(numSuccessedTestCase, numFailedTestCase, isSelfHealing, undefined);
            break;
        case 'dynamic':
            trackingExecuteTestSuite(title, numSuccessedTestCase, numFailedTestCase, isSelfHealing, null, 'dynamic')
            break;
        default:
            break;
    }
...
function trackingExecuteTestCase(testCaseName, status, isSelfHealing, testCaseId) {
    const testCase = findTestCaseById(testCaseId);
    const isContainLoadVars = testCase.commands.some(command => command.name === "loadVars");
    let action = {
...
function trackingExecuteTestSuite(testSuiteId, noSuccessedTestCase, noFailedTestCase, isSelfHealing, isConsole, type) {
    let action = {
        test_suite_id: testSuiteId,
