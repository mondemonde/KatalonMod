...
const renderNewProfile = (profile) => {
    ...
    container.addEventListener("click", async function() {
        setSelectedProfile(this.id);
        const profile = await findProfileByID(this.id);
        ...
        $("#profileTitle").html(profile.title).show();
        //need to reset colResizable first
        $("#profile-variable-grid").colResizable({ disable: true })
            .colResizable({ liveDrag: true, minWidth: 75 });
        //display profile variable to sample-records-grid
        const profileGrid = $("#profile-grid");
        $(profileGrid).empty();
        for (let i = 0; i < profile.variables.length; i++) {
            const profileVariable = profile.variables[i];
